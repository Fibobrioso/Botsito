...
| Anclaje de la vela H4 (00:00 Madrid, 00:00 servidor o 17:00 NY) cambia el sesgo la mitad de los dias; MQL5 `PERIOD_H4` nativo divergiria de Python | F07, F10, F15, F29, F30, F33 | Parametro `anclaje_h4` en estado UNKNOWN/DEFAULT_AMBIGUOUS (A-9) hasta evidencia `modalidad: pantalla` de la configuracion del grafico (F07). F15 (hecho): agregacion por particion de la recta UTC segun el reloj de pared del anclaje (ADR-0005), `anclaje_h4` UNKNOWN sin huso, `--anclaje` explicito en el CLI; verificado con datos reales de las cuatro semanas de cambio de hora y con M1 sintetico 24/7; el golden contra la captura del trader se anade en F07 como test de regresion sobre F15. F10: mientras A-9 siga abierta, el kit dibuja cada caso con los dos anclajes candidatos (Madrid y servidor). F29: H4 agregada desde M1 con el mismo anclaje; prohibido `iTime/iClose(PERIOD_H4)`. F33: comparar ultima H4 cerrada EA vs Python |
