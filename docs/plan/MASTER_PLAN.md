...
| Veto de riesgo solo en MQL5: el backtest Python del mismo dia ejecutaria lo que el EA veto y F34 lo contaria como divergencia | F21, F22, F24, F30 | El veto de la firma es RN-029 (ADR-0026): `firma_perdida_diaria_max` sobre el capital inicial, contado desde el saldo a medianoche CE(S)T (`firma_base_perdida_diaria`, `reloj_dia_riesgo`, ADR-0027), y `firma_perdida_total_max` estatico, los dos sobre equity (`firma_magnitud_vigilada`); mas `firma_mensajes_dia_max` y el margen que impone `firma_apalancamiento`. Hasta el 2026-09-14 esta celda decia "5 % diario sobre equity a medianoche de servidor, 10 % total" SIN FUENTE y con el reloj equivocado. Es regla pura del dominio con `equity`, `balance_al_corte_diario`, `mensajes_enviados_hoy`; se evalua POR TICK (ADR-0028); F24 la ejecuta; F28 la exporta; F30 la cubre; el journal registra cada veto como abstencion |
