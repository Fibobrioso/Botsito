...
- Regimenes de contenido: evidencia inmutable; feedback del trader solo-anadir; especificacion
  y casos versionados citando evidencia o feedback; manifiestos de datos, de transcripcion y de
  fotogramas inmutables (ADR-0005, ADR-0007, ADR-0008).
...
| una ambiguedad de alcance, metodo o herramienta | el **consultor** | `estado: DECIDIDA` con `decision: ADR-NNNN` y `decidida_el` (ADR-0022, 2026-09-12). Tres guardias: el ADR existe, el ADR **nombra** la ambiguedad, y no vale sobre una `bloqueante` ni sobre una que sostenga el default de un parametro. **A-15 y A-16 siguen ABIERTA**: el mecanismo esta, falta el ADR que las cierre, y mientras tanto entran en el cuestionario de la sesion siguiente |
...
| `knowledge/corpus` | manifiesto con hash y huecos (F03); glosario del ASR y manifiestos inmutables de transcripcion (F04); obligatorios y manifiestos inmutables de fotogramas (F05) |
| `knowledge/evidence` | `EvidenceItem` inmutables (F06); cita verificada contra la cruda o el fotograma, `_temas.yaml`, `_contradicciones.yaml` (F07, ADR-0009) |
| `knowledge/_proposals` | propuestas de evidencia con prompt, modelo, contexto, salida sellada y decision humana (F07, ADR-0009); manual y versionado |
...
| `knowledge/cases/kit` | kit de elicitacion (F10, ADR-0011): config, mapa de parametros, dias vistos y un paquete por sesion (cuestionario, ventanas, particiones commiteadas antes de la sesion, hoja del trader) |
...
| F05 | `feature/F05-frame-extraction` | Fotogramas de todo el video a 1 fps sin perdida, con `pts` real e indice inmutable (ADR-0008; antes "densos en tramos con decision") | F03, F04 | integridad del indice; regla de seleccion con `pts` exactos | sin huecos > 2 s en ningun video; fotogramas obligatorios de las marcas no verificables por audio presentes y legibles |
...
| **Fase 4 · Datos** | | | | | |
| F15 | `feature/F15-market-data-ohlc` | OHLC y agregacion con anclaje y huso | F02 | DST; anclaje; golden | H4 reproducible para cualquier `anclaje_h4`; golden del trader en F10 (F07 no encontro fotograma con la apertura de una H4) |
...
| F26 | `feature/F26-fidelity-validator` | Fidelidad contra el trader (holdout-1; umbrales pre-registrados en `PREREGISTRO.md`; cita la exposicion declarada, ADR-0021) | F10, F14, F24 | comparador; holdout; pre-registro | informe reproducible que cita PREREGISTRO.md |
...
en rojo (paso con F04 3b754f1 y F05 f452e6f, revertido). El run de CI que se anota es el del
`docs(state)` (el merge se pushea junto y no tiene run propio). Una rama de auditoria global (sin
numero F) sigue el mismo ritual con tag `stable/F##-auditoria-N` (F## = ultima funcionalidad) y
un informe `docs/validation/AUDITORIA-<fecha>-<tema>.md`. Una rama de trabajo sin numero propio (previos de una
funcionalidad) sigue el mismo ritual con tag `stable/F##-<tema>` (F## = ultima funcionalidad
cerrada; por ejemplo `stable/F05-previos-F07`) y un informe `docs/validation/<tema>.md`.
...
herramienta, metodo) se cierra con `estado: DECIDIDA` y el ADR que la nombra (ADR-0022,
2026-09-12); A-22 es la primera. Lo que queda abierto no es el mecanismo sino su uso: A-15 y
A-16 llevan decididas de hecho desde el 2026-09-09 y siguen ABIERTA, asi que hoy entrarian en
el cuestionario de la sesion 2 y se le preguntaria al trader lo que el consultor ya decidio.
...
|---|---|---|
| Tres particiones reservadas: `holdout-1` (se abre en F26, sesion 2), `holdout-2` (cifra final de fidelidad, una sola apertura), `holdout-3` (correcciones de fase 7). La asignacion se commitea con seed ANTES de la sesion de etiquetado; un test comprueba la fecha del commit. F10 (hecho): guardia de ancestro en git (`commit_que_anadio` + `es_ancestro`) y asignacion inmutable tras el etiquetado (`intacto_desde`) | F10, F14, F26, F34, F35 | Medir la fidelidad final sobre casos ya vistos al corregir |
...
| Test en CI contra el historial de git: cada fichero de `knowledge/evidence/`, `knowledge/feedback/`, etiquetas de casos y manifiestos de datos es byte-identico a su primera version commiteada | F06, F09, F14, F15 (hecho: `tests/contract/test_data_manifest_history.py`), F04 (hecho: `test_transcripcion_history.py`), F05 (hecho: `test_fotogramas_history.py`) | Editar evidencia o feedback saltando los hooks |
...
| Propuestas de LLM en `knowledge/_proposals/` con prompt, modelo, salida y decision humana; `extractor` y `revisado_por` (persona, hoja y metodo) obligatorios en evidencia. F07 (hecho, ADR-0009): `pr-<video>-<t0>-<t1>-<hash8>.yaml` con prompt y sha256, modelo, contexto de la cruda, `items`/`no_consta`, sello `salida_sha256` recomputado por `knowledge validate`; `evidence accept|reject` anotan la decision; 20 propuestas, 341 items aceptados por el usuario el 2026-09-07 | F07 | Perder el rastro de que propuso la IA |
| Cita de evidencia con id de transcripcion y verificada contra la CRUDA: campo opcional `transcripcion` (id `tr-*`) en EvidenceItem sin alterar ids existentes; `evidence new` comprueba `cita_literal` contra `transcript show --capa cruda` de la transcripcion activa (la corregida cambia con cada edicion del glosario bajo el mismo id, asi que no sirve de referencia; las `palabras` solo se citan desde la cruda); condicion previa a la primera evidencia con `transcripcion:`: glosario v2 aprobado, los 5 videos (v1-v5) retranscritos con `--reemplaza-a`, copia de `cruda.jsonl`/`audio.wav` en Drive y v5 subido a Drive con su `drive_id` en `fuentes.yaml`; atribucion de voz por contexto (sin diarizacion, riesgo del brief F04). F07 (hecho, ADR-0009): `evidence/verificacion.py` localiza la cita por tokens en `[t0 - 2 s, t1 + 2 s]` con tiempo real por `palabras` y comodin `[...]`; 341 items (337 con `transcripcion`; los 4 de pantalla anclados a `fr-*`); `verificar_citas` en `knowledge validate` | F07 | Citas que no se pueden localizar ni reproducir |
...
| Manifiestos de datos inmutables con `dataset_id`, proveedor, fecha, `schema_version`; escritura atomica en el grabador de demo; desfase servidor-UTC registrado | F15 (hecho: `data/manifests/`, id con hash del contenido, `reemplaza_a`, guardia de historial y hook), F16, F17 | Re-descargas que cambian datos congelados; ficheros truncados |
...
|---|---|---|
| Tres relojes (trader Europe/Madrid, servidor GMT+2/+3, datos UTC) con desfase variable. **Corregido el 2026-09-14 (ADR-0027)**: el dia de riesgo de la prop firm NO es el del servidor; con FTMO se corta a medianoche CE(S)T, que es el reloj civil del trader, y el tercer reloj desaparece | F11, F15, F17, F28, F31, F33 | F15 (hecho, ADR-0005): `huso_operativa` como parametro; `huso_datos` NO es parametro sino campo fijo de cada manifiesto (una puerta por valor); el reloj de servidor se aproxima como `17:00 America/New_York` (brokers MT5 con DST de Nueva York). F11: `dst_servidor` (`us | eu | ninguno`) y `offset_base_servidor` en categoria `broker`, por ADR. F17: desfase servidor-UTC en cada evento (`TimeTradeServer() - TimeGMT()`; el modulo `MetaTrader5` de Python devuelve `time` en hora de servidor). F28: exportar tabla de transiciones DST del horizonte, no un offset fijo. F31: dia de riesgo = dia civil en `huso_operativa` (`reloj_dia_riesgo = civil_operativa`, ADR-0027; decia "dia de servidor" hasta el 2026-09-14). F17: el desfase y el calendario de horario de verano del servidor de FTMO se miden desde cero (A-28; los de FundedNext no se heredan, y el calendario exige observar la transicion de octubre). F33: pre-vuelo compara offset real con la tabla y aborta |
| Anclaje de la vela H4 (00:00 Madrid, 00:00 servidor o 17:00 NY) cambia el sesgo la mitad de los dias; MQL5 `PERIOD_H4` nativo divergiria de Python | F07, F10, F15, F29, F30, F33 | Parametro `anclaje_h4` en estado UNKNOWN/DEFAULT_AMBIGUOUS (A-9) hasta evidencia `modalidad: pantalla` de la configuracion del grafico. F07 (hecho): ningun fotograma muestra la hora de apertura de una H4; la evidencia registra el reloj del grafico en UTC+2 (`ev-v3-000136-6160fcea`, `ev-v3-000157-b26147c7`); la captura se pide en la sesion 1 (F10). F15 (hecho): agregacion por particion de la recta UTC segun el reloj de pared del anclaje (ADR-0005), `anclaje_h4` UNKNOWN sin huso, `--anclaje` explicito en el CLI; verificado con datos reales de las cuatro semanas de cambio de hora y con M1 sintetico 24/7; el golden contra la captura del trader pasa a F10 (decision del usuario 2026-09-07) como test de regresion sobre F15. F10: mientras A-9 siga abierta, el kit dibuja cada caso con los dos anclajes candidatos (Madrid y servidor). F29: H4 agregada desde M1 con el mismo anclaje; prohibido `iTime/iClose(PERIOD_H4)`. F33: comparar ultima H4 cerrada EA vs Python |
| Parametros de instrumento, broker, prop firm y ejecucion sin hogar (HECHO en F11: nueve parametros medidos contra la demo real) | F11, F24, F28, F31, F33 | Categorias del ADR-0004 (hecho). F11 los puebla citando ADR. F24 simula `stops_level`, `freeze_level`, comision y modo de llenado desde `instrumento`/`broker`. F28: `Params.mqh` (estrategia+instrumento+ejecucion) y `Risk.mqh` (prop_firm) con dos hashes en el journal. F33: `[broker]` en settings, entorno `real`, pre-vuelo contra `SymbolInfo*`/`AccountInfo*`. **F12 (2026-09-11)**: cada parametro con valor que ninguna regla nombra declara su lector en `consumido_por`, y estos tres se apoyaban en esta fila sin estar nombrados, asi que se nombran: F17 y F33 leen `cuenta_pruebas` (F17 mide el reloj del servidor contra ese terminal), F33 lee `cuenta_objetivo`, y F24 arranca el backtest con `saldo_inicial_cuenta`, que no es base de ningun calculo |
| Veto de riesgo solo en MQL5: el backtest Python del mismo dia ejecutaria lo que el EA veto y F34 lo contaria como divergencia | F21, F22, F24, F30 | El veto de la firma es RN-029 (diario) y RN-031 (total, permanente), con el cierre de la posicion viva en RN-030 y la lectura prospectiva -no abrir lo que no cabe- en RN-032 (ADR-0026; desde el 2026-09-16 los cuatro saltan en el limite menos `firma_margen_seguridad`, ADR-0031): `firma_perdida_diaria_max` sobre el capital inicial, contado desde el saldo a medianoche CE(S)T (`firma_base_perdida_diaria`, `reloj_dia_riesgo`, ADR-0027), y `firma_perdida_total_max` estatico, los dos sobre equity (`firma_magnitud_vigilada`); mas `firma_mensajes_dia_max` y el margen que impone `firma_apalancamiento`. Hasta el 2026-09-14 esta celda decia "5 % diario sobre equity a medianoche de servidor, 10 % total" SIN FUENTE y con el reloj equivocado. Es regla pura del dominio con `equity`, `balance_al_corte_diario`, `mensajes_enviados_hoy`; se evalua POR TICK (ADR-0028); F24 la ejecuta; F28 la exporta; F30 la cubre; el journal registra cada veto como abstencion |
| Redondeo fraccion → puntos (0,75 × 137 = 102,75) y lotaje al paso: `Decimal` no lo resuelve frente a MQL5 | F18, F21, F28, F29 | F18: tipo `Puntos(int)` (creado en F15 en `valores.py`; F18 anade la regla), regla escrita en ADR-0029 (geometria en BID; al punto entero mas cercano y, en empate, en contra del bot: el stop se aleja y el objetivo se acerca; lote hacia abajo al paso), test AST que prohibe `float`/`Decimal` en `domain/` salvo `valores.py`. F21: golden con residuo ,5. F28: fracciones como racionales `num/den`; F29: aritmetica en `long` |
| Sesion 1 sin registro, sin reglas ni casos: `RESOLVE_UNKNOWN` sobre parametros inexistentes falla; `LABEL_CASE` sobre casos que F14 aun no creo; la particion holdout debe existir antes de la sesion | F10 | F10 pre-puebla `parametros.yaml` con todos los nombres en `UNKNOWN` (commit `Fuente: ADR-0004`), genera ids de caso, seed y asignacion dev/holdout-1/2/3 con el test de fecha (F14 conserva runner y fixtures). Papel `sesion_feedback` en `fuentes.yaml`, `drive_id` opcional para grabaciones locales, `t0/t1` del feedback contra la duracion de la grabacion. F10 (hecho, ADR-0011): 30 UNKNOWN de estrategia (incluidas las decisiones que la evidencia no nombra: dias operables, noticias, spread maximo, topes de perdida, que hacer sin regla) y objetivo de feedback `paquete` para la precondicion de ceguera; `knowledge/cases/kit/<sesion>/{ventanas,particiones}.yaml` con seed y guardia de ANCESTRO en `knowledge validate` (no de fecha) e inmutabilidad tras el primer `LABEL_CASE`; grabaciones de sesion como `videos` sin `drive_id`; `t1` contra `duracion_s` |
| Dos ficheros de spec sin version ni hash comun (HECHO en F11: `spec_manifest.yaml` con semver y hash canonico sobre los TRES ficheros, huso incluido) | F11 | `spec_manifest.yaml` con `spec_version` semver y hash canonico sobre `parametros.yaml + strategy_spec.yaml + glossary.yaml`; `strategy_spec.yaml` referencia parametros por nombre, nunca por valor |
| Restricciones del broker fuera de la spec (HECHO en F11: RN-026 abstencion por stops level, RN-027 redondeo del lote a la baja) | F11, F24, F31 | Seccion "restricciones de ejecucion" en la spec con decision explicita (rechazo por stops level = abstencion, nunca aproximacion); F24 las simula; F31 solo implementa lo que la spec dice |
...
| Tipos que faltan: `enum`, `booleano`, `puntos`, `minutos`, `lotes` (HECHO en F11) | F11, F28 | F11 anade los tipos con `opciones`; F28 los exporta como `enum`/`bool`/`long` |
| Modelo de llenado en la spec pero no en el registro (HECHO en F11: `modelo_llenado` y `latencia_ms`) | F11, F24, F27 | Parametro `categoria: ejecucion`, `tipo: enum` (`al_tocar` / `cruce_mas_latencia`) con `latencia_ms`; dos informes en F27 |
...
| F05 sin insumo definido: "tramo con decision" no dice quien lo declara; el HTML habla de intervalos del manifiesto y el brief F04 de la transcripcion | F05 (hecho, ADR-0008) | RESUELTO midiendo: cualquier regla de palabras clave sobre las crudas marca el 46-99 % de cada video (ahorro 7-29 %), asi que F05 extrae TODO el video a 1 fps sin perdida (decision del usuario 2026-09-05: maxima fidelidad sin restriccion de recursos) y el concepto "tramo con decision" desaparece. Obligatorios en `knowledge/corpus/fotogramas_obligatorios.yaml` (V3 0:28:56, V2 0:33:21, V4 0:12:30; validados contra el indice activo). Configuracion del grafico (A-9): sin instante conocido; el informe de F05 lista candidatos buscados en las crudas y F07 elige el fotograma |
| Detalle vivo de F05–F35 solo en `MASTER_PLAN.html` con nombres obsoletos (`botv3 corpus frames --dense`, `botv3 kb ...`) | F04, F05 (hecho), F07+ | Trasladar el detalle por funcionalidad al Markdown al abrir cada una (el HTML queda como instantanea) |
...
## Change Log del plan
- 2026-09-14 · Rama `trabajo/ftmo-y-arquitectura`: LA FIRMA PASA A SER FTMO 2-Step Swing (ADR-0026), porque FundedNext no admite bots desde 50.000. Los topes de la firma entran en el registro (`firma_*`) y en la spec (RN-029, `gate`); el tipo Swing no restringe noticias, asi que RN-028 se DESCARTA y `filtro_noticias` vuelve a `no` (enmienda de ADR-0022) y el calendario de noticias sale del camino critico. El dia de riesgo pasa a ser civil (ADR-0027) y H.2 deja de decir "medianoche de servidor". Y los cuatro ADR de arquitectura que la auditoria del 2026-09-13 echo en falta: el reloj del motor en tres fases con el riesgo por tick (ADR-0028), el lado del precio y el redondeo (ADR-0029) y el reparto arbol generico + primitivas a mano (ADR-0030, que es la lectura con que se interpreta la tabla A para F18-F22). A-17 y A-19 DECIDIDAS, A-27 y A-28 abiertas (mediciones en la demo de FTMO). spec_version 10.2.0 -> 11.1.0 (11.0.0 con la regla nueva y la que cambia de sentido; 11.0.1 con las correcciones de la auditoria de cierre; 11.1.0 al partir el cierre de la firma a RN-030 con su parametro, a peticion del consultor al validar).
- 2026-09-12 (4) · F13 spec-documents: `docs/spec/` se GENERA desde `knowledge/spec/` (`botsito spec docs`) y la guardia compara el texto entero; muere el §2 del acta de la sesion 1 y el recuento de `knowledge/spec/README.md`. La hoja del trader entra en la biblioteca (`botsito kit hoja`). El vocabulario de la forma ejecutable gana `tokens`: los sujetos de geometria y los estados dejan de ser texto libre, que era la mitad abierta de ADR-0019 -la auditoria de cierre demostro SEIS formas distintas de colar un valor de negocio en `forma`, incluida `tope: 9.5`-. spec_version 10.0.0 -> 10.1.1.
- 2026-09-12 (3) · ADR-0022: EL BOT NO OPERA NOTICIAS en la cuenta fondeada, y nace el estado `DECIDIDA` para las ambiguedades que decide el consultor. El trader SI las opera -en cuentas propias, y su estrategia funciona dentro del evento- pero el bot va a una cuenta que puede prohibirlo, con la cuenta como sancion. RN-021 se parte y las noticias van a RN-028 (`gate`), que nace con `pendiente_definicion: A-17` porque nadie ha leido aun el reglamento. A-17 se parte en dos: la verificacion (ABIERTA) y la decision de alcance (A-22, DECIDIDA). ADR-0023: el registro de feedback gana `recibido_el` y `procedencia`.
- 2026-09-12 (2) · ADR-0021: QUE CUENTA COMO ABRIR UN HOLDOUT. Llevaba desde F01 escrito "un holdout abierto queda quemado" sin decir que es abrir, quien autoriza ni que hacer ante una exposicion, y la exposicion ya habia ocurrido: el material de mayo trae el PnL dia a dia de los 19 dias, trece de ellos reservados. Abrir = leer sus etiquetas o medir una cifra del bot sobre esos dias; ver el resultado agregado NO abre pero SI se declara. La exposicion del 2026-09-11 no quema y queda registrada en `docs/validation/HOLDOUT-EXPOSICIONES.md`, nuevo. Nace tambien `docs/validation/PREREGISTRO.md`, que el plan exigia en presente desde §G y no existia: va VACIO a proposito, y mientras lo este no se abre ningun holdout. F26 hereda citar la exposicion; F14, implementar de verdad la guarda de `tests/conftest.py`, que hoy es un stub.
...
- 2026-09-07 · F07 cerrada (ronda 2; el usuario acepto los 341 items): filas H "Propuestas de LLM" y "Cita contra la CRUDA" marcadas como hechas; fila H.2 "Anclaje de la vela H4" y tabla A (F15) corregidas: el golden H4 del trader pasa a F10; fila H.2 "Previos y entradas de F07" cerrada; tabla B con `knowledge/_proposals` y la evidencia verificada
- 2026-09-06 · F07 abierta (rama `feature/F07-evidence-extraction`): brief con revision de diseno de agente (bloqueantes: `evidence` no importa `corpus`, comodin `[...]`, tiempo por `palabras`, `material_adicional` solo desde video, `revisado_por` con metodo, sello de propuestas); ADR-0009; fila H "Propuestas de LLM" y "Cita contra la CRUDA" implementadas; golden H4 sobre F15 a F10 (ningun fotograma muestra la hora de apertura de una H4); ronda 1 (propuestas) en espera de la decision del usuario
- 2026-09-06 · previos de F07 (rama `feature/F07-previos`): fila H.2 marcada como hecha (Drive completado por el usuario el 2026-09-06) (glosario v2, `hotwords` descartado con medicion, huella sin GPU, 5 videos retranscritos); ADR-0007 con enmienda; §F con el tag de una rama de trabajo sin numero propio (`stable/F##-<tema>`)
- 2026-09-05 · auditoria global de la estructura (rama `feature/F05-auditoria-estructura`): §0 y B con fotogramas (ADR-0008); §F con la regla del HANDOFF (en la rama, nunca en main tras el tag), el run de CI que se anota y el ritual de una rama de auditoria; H con F04/F05 en la fila de tests de historial y 5 videos + v5 en Drive en la fila de cita; H.2 con F05 hecho y fila nueva "Previos y entradas de F07"; Change Log reordenado
- 2026-09-05 · F05: tabla A fila F05 reescrita (cobertura completa a 1 fps sin perdida en lugar de tramos con decision; ADR-0008) tras medir sobre las crudas que una regla de tramos no filtra (46-99 % marcado) y por instruccion del usuario (maxima fidelidad sin restriccion de recursos); H.2 fila F05 resuelta; brief con revision de diseno por agente (7 bloqueantes: `fps=1` elegia el fotograma equivocado y reescribia el `pts`, huecos vacuos, referencias heredadas citables, etc.), todos aplicados
