...
## Current Feature
F12 spec-semantic-validator (rama `feature/F12-spec-semantic-validator`, brief en `docs/plan/features/F12-spec-semantic-validator.md`): que la spec no pueda decir cosas incoherentes entre si y que las reglas dejen de ser prosa. ABIERTA el 2026-09-10 con el brief escrito y la REVISION DE DISENO PENDIENTE: no se programa hasta cerrar D1 (que forma ejecutable), D2 (convertir las 24 reglas o un piloto), D3 (si una regla sin parametros es legitima) y D4 (precedencia entre reglas). El brief parte del DELTA: la auditoria de F11 ya construyo ocho comprobaciones semanticas que el plan asignaba aqui.
