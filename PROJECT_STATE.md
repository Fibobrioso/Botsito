...
## Current Feature
F07 · evidence-extraction · WAITING_FOR_USER_VALIDATION, ronda 1 de 2 (rama `feature/F07-evidence-extraction`; informe `docs/validation/F07-evidence-extraction.md`; hoja `docs/validation/anexos/F07-evidence-extraction/revision.html`; ADR-0009; cierre con tag `stable/F07` tras la ronda 2)
...
- F07 · evidence-extraction, ronda 1 (2026-09-06): 20 propuestas en `knowledge/_proposals/` con 341 items propuestos y 91 `no_consta`, todas selladas; 0 items en `knowledge/evidence/` hasta la decision del usuario sobre la hoja de revision
...
## Next Action
El usuario decide sobre la hoja de revision de F07 (aceptar por lotes / rechazar con motivo / `fotograma visto` en los 7 items de pantalla o ambas; recall humano del tramo V4 0:05-0:15). Ronda 2: `evidence accept`/`reject` item a item con `revisado_por` = decision del usuario, commit de la evidencia (inmutable), `evidence contradictions`, golden `test_golden_citas_f07`, hechos de PROJECT_STATE -> ids, auditoria de cierre con dos agentes, informe final, parada corta, ritual §F con tag `stable/F07`.

Historial: Abrir F07 evidence-extraction con el metodo supervisado (brief desde MASTER_PLAN H.2 fila "Previos y entradas de F07" y tabla A -> revision de diseno por agente -> construir -> auditoria de cierre con dos agentes -> informe WAITING_FOR_USER_VALIDATION). Entradas: cita de audio contra la CRUDA activa (`transcript show --capa cruda`), cita de pantalla `fr-<id>/<t_ms>` via `referencias_conocidas` conectada a `validar_contra_manifiesto` y a `evidence new`, hechos ya leidos en "Hechos del corpus pendientes de evidencia", `dudas` del glosario (25 segmentos) revisadas al citar.
