...
## Current Phase
FASE 2 · Retroalimentacion del experto. SESION 1 CELEBRADA el 2026-09-09 (2 h 27 min, video v6): las 27 preguntas del cuestionario, las 3 adicionales y las 14 confirmaciones respondidas, y las doce ambiguedades A-1..A-12 RESUELTAS con feedback del trader. El etiquetado de casos lo entrega el trader como backtest: MAYO llego el 2026-09-11 (68 operaciones, en el corpus) y JUNIO queda DESCARTADO por decision del consultor el 2026-09-12, asi que la biblioteca de casos se construye solo con mayo: 19 dias, de los que 6 son `dev` y 13 holdout. Siguiente: cerrar F12

## Current Feature
F12 spec-semantic-validator (rama `feature/F12-spec-semantic-validator`, brief en `docs/plan/features/F12-spec-semantic-validator.md`): que la spec no pueda decir cosas incoherentes entre si y que las reglas dejen de ser prosa. ABIERTA el 2026-09-10. D1 y D2 CERRADAS por el consultor: la forma ejecutable son PREDICADOS NOMBRADOS -definidos una vez, con su cita y sus parametros, DENTRO de strategy_spec.yaml (un cuarto fichero rompia el contrato del hash, ADR-0019); F22 los implementa. NO heredan gratis las guardias de F11: cada una hubo que ampliarla a mano, y la auditoria de cierre encontro tres que seguian sin mirar el vocabulario- y se convierten primero CUATRO reglas piloto (RN-003, RN-006, RN-014, RN-020) antes que las veinte restantes. D3 y D4 CERRADAS con la evidencia de la revision (tres agentes + un auditor del arreglo): D3, una regla vigente sin parametros NO es legitima -RN-009 y RN-018 escondian dos cardinalidades de negocio en la prosa-; D4, la precedencia va por CLASE y no por orden de fichero (ADR-0018). La revision encontro ademas SIETE DEFECTOS VIVOS en la spec ya validada, dos de ellos capaces de costar dinero, arreglados en 36e4c65 y corregidos a su vez en 925d3f6 tras auditar el arreglo. ADR-0019 fija la forma ejecutable: predicados CON ARGUMENTOS, ligadura de variables y cuatro cosas con nombre, todo DENTRO de strategy_spec.yaml -un cuarto fichero rompia el contrato del hash-. Brief reescrito: tenia tres errores de hecho y afirmaba que un predicado hereda gratis las ocho guardias, que es falso. HECHO desde entonces: el piloto de cuatro reglas y LAS 24 VIGENTES con forma ejecutable (22 predicados, 13 acciones, 2 efectos, 6 hechos, 3 acumuladores), y el 2026-09-11 el acuerdo final del lotaje (ADR-0020: el 0,5 % se mide EN el nivel 0,8, no sobre la caja completa), que invierte RN-012 y sube el lote un 25 %. HECHO tambien: los parametros sin lector (campo `consumido_por` y guardia), el comando `spec check`, los ids R-NN explicitos, la auditoria de cierre con dos agentes -34 hallazgos de documentacion y 23 de codigo, aplicados- y el informe `docs/validation/F12-spec-semantic-validator.md`. F12 queda WAITING_FOR_USER_VALIDATION. El brief parte del DELTA: la auditoria de F11 ya construyo ocho comprobaciones semanticas que el plan asignaba aqui. PENDIENTE DE HIGIENE: este fichero arrastra ~320 lineas duplicadas desde a6fdcf1 (todo lo que va de `## Completed Phases` en adelante esta dos veces; manda el bloque de arriba, el de abajo es la copia vieja de main y solo el Change Log del final sigue vivo).

## Current Branch
feature/F12-spec-semantic-validator

## Stable Main State
b62f4aa · merge de F11 strategy-spec-schema (tag stable/F11), validada tras una auditoria de catorce puntos. make check verde: 607 casos (425 funciones), 4 contratos, mypy strict, state/config/knowledge validate. spec 3.0.1 · 54 parametros (47 confirmados, 1 con default nuestro, 6 UNKNOWN a proposito) · 27 reglas (24 vigentes) · 19 ambiguedades (7 abiertas).
...
- F12 · spec-semantic-validator · informe `docs/validation/F12-spec-semantic-validator.md` · WAITING_FOR_USER_VALIDATION · cierre con tag `stable/F12`
...
- knowledge/spec/parametros.yaml (LA puerta de los parametros; 52 parametros, 46 con valor y 6 UNKNOWN a proposito: `huso_operativa` = Etc/GMT-2 por ADR-0012, que enmienda a ADR-0005; los de estrategia citan al trader y los de entorno un ADR) · `knowledge/spec/strategy_spec.yaml` (27 reglas) · `knowledge/spec/glossary.yaml` · `knowledge/spec/spec_manifest.yaml` (semver + hash sobre los tres) · src/botsito/config/registro.py · src/botsito/domain/valores.py
...
- ADR-0019 la forma ejecutable de una regla: predicados con argumentos, ligadura, y cuatro cosas con nombre — ACTIVE
...
- F10: la fecha de la sesion 1 (`2026-09-15`) es provisional (regenerar con `kit build --sesion <fecha>-sesion-01 --seed 20260915` y commitear ANTES de la sesion); el paquete exige la confirmacion escrita del trader de que no ha visto mayo ni junio de 2026 (si la niega, `vistos.yaml` y otro mes con `data download`); `mapa_parametros.yaml` sigue con sus `opciones` ademas del registro: no se pudieron unificar sin romper la reproducibilidad del paquete historico de la sesion 1, asi que hay un test que cruza las dos listas y la unificacion queda para F13; el primer dia laborable de cada mes solo entra si el mes anterior esta congelado y es contiguo.
...
## Tests Currently Passing
425 funciones de test (607 casos; parametrizadas x3, x5, x6, x7, x8, x9, x11, x13, x15, x18, x19 y x22) · unit: project_state, project_state_rutas, adr, tree, cli, cli_data, valores, velas, registro, ajustes, inventario, evidence, feedback, yaml_estricto, dukascopy, agregacion, agregacion_dst, dataset, golden_ohlc, comun, audio, transcripcion, pipeline_transcripcion, motor_prompt, verificacion, propuestas, fotogramas, retrieval, kit, spec · integration: fotogramas_ffmpeg · contract: import_contracts, no_business_literals, repository_integrity, registro_accessors, evidence_history, feedback_history, data_manifest_history, transcripcion_history, fotogramas_history, golden_citas_f07 (40 referencias contra la evidencia real), golden_consultas_f08 (15 consultas; con y sin `data/`), kit_particiones (guardia de ancestro), hoja_sesion_docx (OOXML valido, citas del paquete, sin fuga de holdout) · 4 contratos import-linter KEPT · mypy strict OK (src + tests)
...
## Next Feature
F13 / F14, que se abren con F11 igual que F12 (MASTER_PLAN §D). F14 (biblioteca de casos) espera el backtest de mayo y junio del trader; F13 recoge lo que queda de `mapa_parametros.yaml` y el filtro de `feedback pending`.
...
## Last Stable Commit
b62f4aa · merge: F11 strategy-spec-schema, auditada punto por punto antes de validar · tag stable/F11
