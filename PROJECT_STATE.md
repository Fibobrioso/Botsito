...
## Current Feature
Previos de F07 (glosario v2, hotwords medido y descartado, huella sin GPU, retranscripcion v1-v5, copia en Drive) · WAITING_FOR_USER_VALIDATION (rama `feature/F07-previos`; informe `docs/validation/F07-previos.md`; cierre con tag `stable/F07-previos`)
...
- Previos de F07 (2026-09-06): `docs/validation/F07-previos.md`; 5 manifiestos `tr-*` nuevos; pendiente del usuario: arrastrar `data/drive_staging/` a Drive y anotar el `drive_id` de v5
...
- **2026-09-04 · Geometria del riesgo y gestion del stop.** "Se traza un 1:3 inicialmente; con eso
  ...
  - Material del 2026-09-05 (v5, `tr-v5-large-v3-int8-float16-01a1ae03`): 0:03:21 "si yo protejo a
...
- Copia de seguridad de las crudas activas y los WAV fuera de esta maquina: carpeta de Drive `1zYZjUAYMoine0RILKg2ZJyzcLz5-1p-R` ("transcripciones (crudas, Bot v3)") con SHA256SUMS, LEEME y manifiestos ya subidos (2026-09-06); crudas, WAV y v5 (625 MiB) listos en `data/drive_staging/` para que el usuario los arrastre (dueno: usuario). Sin ella, otra maquina solo valida esquema e historial (o retranscribe: ~1 h de GPU).
...
- v5 (`corpus/Estrategia del trader/2026-09-05 21-03-59.mkv`, 121,5 MB) NO esta en Drive: copia lista en `data/drive_staging/` (sha256 en SHA256SUMS); el usuario lo arrastra a la carpeta de Drive y anota su `drive_id` en `fuentes.yaml`.
